...
export function useJornada4SegmentAttention({
  ...
  const tick = useJornada4Tick(enabled && Boolean(userId && planilla));
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const entropyLike = fresh.filter(
          e => e.type === "entropia" || e.type === "auto_apertura" || e.type === "day_rollover_entropia"
        );
    ...
    void run();
  }, [enabled, userId, tick, planilla?.updatedAt, planilla?.segmentos?.length]);
}
