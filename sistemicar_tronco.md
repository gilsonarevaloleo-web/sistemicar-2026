...
**Jornada 4 — Dual Kernel (propuesta / estudio):** nueva ruta `/jornada-v4` que **no** monta anillo, escalera, disciplina ni TTS. Voz GPS opcional por clips (`HTMLAudioElement`, kill-switch default OFF, fuera de kernels y del tick). Base = lógica pura de desglosador Conquista + Situacional + PS, con wall-clock en background. Cantera de libs del tronco; runtime recortado para no congelar. Ver `docs/JORNADA4_DUAL_KERNEL.md` y `public/docs/BRIEF_JORNADA4_DUAL_KERNEL.md`.
